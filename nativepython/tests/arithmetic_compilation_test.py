...
class TestArithmeticCompilation(unittest.TestCase):
    ...
    def test_can_compile_all_register_types(self):
        ...
        registerTypes = [Int8, Int16, Int32, Int64, UInt8, UInt16, UInt32, UInt64]

        ones = [T(1) for T in registerTypes]
        twos = [T(2) for T in registerTypes]
        ...
        for T1 in registerTypes:
            for T2 in registerTypes:
                def add(x: T1, y: T2): return x + y
                def sub(x: T1, y: T2): return x - y
                def mul(x: T1, y: T2): return x * y
                def div(x: T1, y: T2): return x / y
                def mod(x: T1, y: T2): return x % y
                def less(x: T1, y: T2): return x < y
                def greater(x: T1, y: T2): return x > y
                def lessEq(x: T1, y: T2): return x <= y
                def greaterEq(x: T1, y: T2): return x >= y
                def bitand(x: T1, y: T2): return x & y
                def bitor(x: T1, y: T2): return x | y
                def bitxor(x: T1, y: T2): return x ^ y
                def neq(x: T1, y: T2): return x != y
                def rshift(x: T1, y: T2): return x >> y
                def lshift(x: T1, y: T2): return x << y
                def pow(x: T1, y: T2): return x ** y
                ...
                for op in [add, sub, mul,
                    ...
                    self.assertEqual(op(T1(1), T2(2)), compiledOp(1,2), (T1, T2, op.__name__))
